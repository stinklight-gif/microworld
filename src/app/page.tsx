...
export default function Home() {
  ...
  const handleExport = useCallback(() => {
    setWorld((prev) => {
      const w = { ...prev, running: false };
      const zip = new JSZip();

      // 1. ledger.jsonl — all events as newline-delimited JSON
      const ledger = w.events.map((e) => JSON.stringify({
        id: e.id, tick: e.tick, type: e.type,
        agents: e.agents, message: e.message, data: e.data,
      })).join("\n");
      zip.file("ledger.jsonl", ledger);

      // 2. stats.json — full stats history
      zip.file("stats.json", JSON.stringify(w.statsHistory, null, 2));

      // 3. summary.md — narrative report
      const alive = Object.values(w.agents).filter((a) => a.alive);
      const dead = Object.values(w.agents).filter((a) => !a.alive);
      const stats = w.statsHistory;
      const totalTrades = stats.reduce((s, t) => s + t.trades_executed, 0);
      const totalBirths = stats.reduce((s, t) => s + (t.births || 0), 0);
      const totalDeaths = stats.reduce((s, t) => s + t.deaths, 0);
      const totalHires = stats.reduce((s, t) => s + (t.hires || 0), 0);
      const lastGini = stats.length > 0 ? stats[stats.length - 1].gini_coefficient : 0;
      const lastHealth = stats.length > 0 ? stats[stats.length - 1].avg_health : 0;
      const sorted = [...alive].sort((a, b) => b.net_worth - a.net_worth);
      const dynastyList = Object.values(w.dynasties);
      const topDynasties = [...dynastyList].sort((a, b) => b.members.length - a.members.length).slice(0, 5);

      const summary = [
        `# Microworld Run Summary`,
        ``,
        `**Experiment:** ${w.activeExperiment?.name ?? "Baseline"}`,
        `**Ticks:** ${w.tick}`,
        `**Date:** ${new Date().toISOString()}`,
        ``,
        `## Population`,
        `- Initial: ${w.initialPopulation}`,
        `- Final alive: ${alive.length}`,
        `- Total deaths: ${totalDeaths}`,
        `- Total births: ${totalBirths}`,
        `- Farmers: ${alive.filter((a) => a.type === "farmer").length}`,
        `- Builders: ${alive.filter((a) => a.type === "builder").length}`,
        `- Energists: ${alive.filter((a) => a.type === "energist").length}`,
        `- Generalists: ${alive.filter((a) => a.type === "generalist").length}`,
        ``,
        `## Economy`,
        `- Gini coefficient: ${lastGini.toFixed(3)}`,
        `- Average health: ${lastHealth.toFixed(1)}`,
        `- Total trades executed: ${totalTrades}`,
        `- Total hires: ${totalHires}`,
        ``,
        `## Top 5 Agents (by net worth)`,
        ...sorted.slice(0, 5).map((a, i) =>
          `${i + 1}. **${a.id}** (${a.type}) — net worth: ${Math.round(a.net_worth)}, health: ${a.health}, children: ${a.children_ids.length}, employees: ${a.employee_ids.length}`
        ),
        ``,
        `## Bottom 5 Agents`,
        ...sorted.slice(-5).reverse().map((a, i) =>
          `${i + 1}. **${a.id}** (${a.type}) — net worth: ${Math.round(a.net_worth)}, health: ${a.health}, ticks starving: ${a.ticks_needs_unmet}`
        ),
        ``,
        `## Dynasties (${dynastyList.length} total)`,
        ...topDynasties.map((d) =>
          `- **${d.founder_id}**: ${d.members.length} living, ${d.total_ever} total`
        ),
        ``,
        `## Dead Agents: ${dead.length}`,
        ``,
        `---`,
        `*Generated by Microworld Simulation*`,
      ].join("\n");
      ...
      zip.generateAsync({ type: "blob" }).then((blob) => {
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = `microworld_${w.activeExperiment?.id ?? "baseline"}_t${w.tick}.zip`;
        a.click();
        URL.revokeObjectURL(url);
      });
